...
pub fn route(method: &Method, path: &str, state: &AppState, body: &[u8]) -> (u16, Value) {
    match (method, path) {
        ...
        (&Method::Get, "/api/modem/schedule-reboot") => device_ext::schedule_reboot_get(state),
        (&Method::Put, "/api/modem/schedule-reboot") => device_ext::schedule_reboot_set(state, body),
        (&Method::Get, "/api/device/schedule-reboot") => device_ext::schedule_reboot_get(state),
        (&Method::Put, "/api/device/schedule-reboot") => device_ext::schedule_reboot_set(state, body),
